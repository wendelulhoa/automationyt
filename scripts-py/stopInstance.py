...
data_folder = "sessions-configs/stop_sessions"
        ...
        if filename.endswith(".json"):
            ...
            with open(file_path, 'r') as file:
                ...
                if session_id:
                    ...
                    try:
                        subprocess.run(["scripts-sh/stop_instance.sh", session_id], check=True)
                        # Exclui o arquivo JSON após iniciar a instância
                        os.remove(file_path)
                        
                        print(f"Instância finalizada para sessão {session_id}")
                    except subprocess.CalledProcessError as e:
                        print(f"Erro ao finalizar instância para sessão {session_id}: {e}")
