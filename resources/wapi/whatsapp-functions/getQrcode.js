...
export async function getQrcode(sessionId) {
    try {
        ...
        for (let index = 0; index < 20; index++) {
            const response = await page.evaluate(async () => {
                try {
                    ...
                    return {qrCode: elQrcode ? elQrcode.getAttribute('data-ref') : null};
                } catch (error) {
                    return {success: false, error: error.message, qrCode: null};
                }
            });
            
            // Obter o QR Code e se a conexão foi estabelecida
        ...
        return {success: (qrCode != null), qrCode: qrCode, message: qrCode ? 'QR Code obtido' : 'QR Code não obtido'};
    } catch (error) {
        console.log(error);
