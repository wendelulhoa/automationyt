...
window.WAPIWU.setGroupDescription = async (groupId, description) => {
    try {
        ...
        var c = await require("WAWebDBGroupsGroupMetadata").getGroupMetadata(
            groupId
        );
...
window.WAPIWU.setGroupProperty = async (groupId, type, value) => {
    try {
        ...
        const types = {
            1: constantsType.GROUP_SETTING_TYPE
...
window.WAPIWU.getGroupInviteLink = async (groupId) => {
    try {
        var codeInvite =
            await require("WAWebMexFetchGroupInviteCodeJob").fetchMexGroupInviteCode(
                groupId
            );
...
window.WAPIWU.createGroup = async (name, participants = []) => {
    try {
        ...
        const metadata =
            await require("WAWebDBGroupsGroupMetadata").getGroupMetadata(
                response._serialized
            );
